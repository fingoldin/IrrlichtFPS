...
class FancyShader : public Shader
{
public:

	FancyShader(Core * core) : Shader(core)
	{
		vertex = "fancyshader.vert";
		pixel = "fancyshader.frag";
	...
	virtual void createShader()
	{
		...
		id = gpu->addHighLevelShaderMaterialFromFiles(vertex.c_str(), "main", irr::video::EVST_VS_3_0,
			pixel.c_str(), "main", irr::video::EPST_PS_3_0,
	...
	virtual void OnSetConstants(irr::video::IMaterialRendererServices * services, irr::s32 userData)
	{
		...
		irr::scene::ISceneManager * smgr = core->getSmgr();
		...
		//core->getDevice()->getLogger()->log(std::to_wstring(shininess).c_str());
		...
		for (size_t i = 0; i < lightNodes.size(); i++)
		{
			...
			case irr::video::ELT_POINT:
				if (plights < FANCY_SHADER_MAX_POINT_LIGHTS) {
			...
			case irr::video::ELT_DIRECTIONAL:
				if (dlights < FANCY_SHADER_MAX_DIR_LIGHTS) {
